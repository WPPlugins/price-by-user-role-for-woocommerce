	...
	function admin_notices() {
		...
		?><div class="error"><p><?php
			echo '<div class="message">'
				. sprintf( __( 'Free plugin\'s version is limited to only one "price by user role per products settings" product enabled at a time. You will need to get <a href="%s" target="_blank">Price by User Role for WooCommerce Pro</a> to add unlimited number of "price by user role per product settings" products.', 'price-by-user-role-for-woocommerce' ), 'https://wpcodefactory.com/item/price-user-role-woocommerce/' )
				. '</div>';
		?></p></div><?php
	}
	...
	function create_meta_box() {
		...
		foreach ( $this->get_meta_box_options() as $option ) {
			...
			if ( $is_enabled ) {
				if ( 'title' === $option['type'] ) {
					$html .= '<tr>';
					$html .= '<th colspan="3" style="text-align:left;">' . $option['title'] . '</th>';
					$html .= '</tr>';
				} else {
					$custom_attributes = '';
					...
					$maybe_tooltip = ( isset( $option['tooltip'] ) && '' != $option['tooltip'] ) ?
						' <img style="display:inline;" class="alg-question-icon" src="' . alg_wc_price_by_user_role()->plugin_url() . '/assets/images/question-icon.png' . '" title="' . $option['tooltip'] . '">' :
						'';
	...
	function get_meta_box_options() {
		...
		if ( $_product->is_type( 'variable' ) ) {
			...
			foreach ( $available_variations as $variation ) {
				...
				$products[ $variation['variation_id'] ] = ' (' . alg_get_product_formatted_variation( $variation_product, true ) . ')';
			}
		} else {
	...
	function get_section_settings() {
		$settings = array(
			...
			array(
				...
				'desc'     => '<strong>' . __( 'Enable', 'price-by-user-role-for-woocommerce' ) . '</strong>',
			...
			array(
				...
				'desc'     => __( 'If per product settings is enabled, you can choose which roles to show on product\'s edit page. Leave blank to show all roles.', 'price-by-user-role-for-woocommerce' ),
				'type'     => 'multiselect',
